...
- **Cleanup/** — `CleanupRule` (matcher, risk: safe / review / protected, recovery, one-sentence explanation) and `CleanupRuleset.default(home:)` (the built-in catalog), `CleanupAnalyzer` (off-main pass per scan → `CleanupReport`: the deepest path rule owns a subtree, nested items are never double-counted, file patterns skip claimed items (so every protected area) and packages; `verdict(for:)` for the inspector). Analysis only: nothing is ever deleted
...
- **Views/** — `ContentView` (NavigationSplitView: sidebar tree + detail + inspector, toolbar), `ResultsView` (breadcrumb, treemap or sunburst, legend per color mode; or `CleanupView`, the suggestions list, always in physical size), `WelcomeView` (volume list with usage rings, NSOpenPanel, folder drop), `ScanProgressView`, `DirectoryTreeView` (outline; row icons and share bars use the chart's colors via `SidebarColors`), `DetailPanelView` (grouped Form: metadata + category breakdown), `LiquidGlass` (glass helpers gated on macOS 26, material fallback on 15)
